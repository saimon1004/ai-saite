/* ============================================================================
   ...
   ----------------------------------------------------------------------------
   SAIMON fork (2026-07-20, from ~/projects/tools/mumbu-scroll-world/site/, MIT):
   adds `heroMode: true` — embeds the world as a HERO SECTION of a longer page
   instead of a full-page takeover. In hero mode the stage/copylayer live inside
   a position:sticky viewport (not position:fixed), page chrome (topbar / nav /
   progress bar / route dots) is not built, the html/body style override is
   skipped, and all scroll math is offset by the container's document position,
   so normal page sections flow in naturally after the flight ends.
   ----------------------------------------------------------------------------
...
function mountScrollWorld(container, config) {
  ...
  const HERO = !!config.heroMode;
  const N = SECTIONS.length;
  if (!N) return;

  injectCSS(HERO);
  container.classList.add('sw-root');
  if (HERO) container.classList.add('sw-hero');
  ...
  if (HERO) {
    // Hero mode: everything visual lives in a sticky viewport; the container
    // itself is the scroll spacer (its height is set in layout()). No page
    // chrome (scrollbar / topbar / route) — the host page owns those.
    const viewport = el('div', 'sw-viewport');
    [sky, stage, copylayer, hint].forEach(n => viewport.appendChild(n));
  ...
  let baseY = 0;   // hero mode: the container's document-top offset — all scroll math is relative to it
  ...
  function layout() {
    ...
    totalW = off;
    if (HERO) {
      container.style.height = (totalW * vh + vh) + 'px';   // container is the spacer; +1vh so the last flight completes
      baseY = container.getBoundingClientRect().top + (window.scrollY || window.pageYOffset);
    } else {
  ...
  function jumpTo(i) {
    ...
    window.scrollTo({ top: baseY + seg.start + (seg.end - seg.start) * 0.5, behavior: reduce ? 'auto' : 'smooth' });
  }
  ...
  function read() {
    let y = (window.scrollY || window.pageYOffset) - baseY;
    // Above the hero (in-flow header etc.): hold the first frame, don't pre-fade it.
    if (HERO && y < 0) y = 0;
...
function injectCSS(heroMode) {
  ...
  ${heroMode ? '' : 'html,body{margin:0;background:var(--sw-bg,#F5EDE0);overflow-x:hidden;}'}
  /* Hero mode: the world lives in a sticky viewport inside the (spacer-height)
     container — page-level fixed positioning becomes viewport-local absolute. */
  .sw-hero{position:relative;}
  .sw-hero .sw-viewport{position:sticky;top:0;height:100vh;height:100dvh;overflow:hidden;}
  .sw-hero .sw-sky,.sw-hero .sw-stage,.sw-hero .sw-copylayer{position:absolute;inset:0;}
  .sw-hero .sw-hint{position:absolute;}
